...
local MINOR_VERSION = tonumber(("$Revision: 67202 $"):match("%d+")) or 0
...
function frame:QUEST_LOG_UPDATE()
	...
	if not lib.firstDone then
		changed = true
		lib.firstDone = true
		frame:PARTY_MEMBERS_CHANGED()
	end
	
	if changed or not lib.firstDone then
		...
		for _, uid in pairs(lib.quest_ids) do
			...
			if lib.quest_objectives[uid] then
				for desc, o in pairs(lib.quest_objectives[uid]) do
					...
					local got = isrep and lib:GetReactionLevel(o[1]) or o[1]
					local need = isrep and lib:GetReactionLevel(o[2]) or o[2]
...
function lib:ShowQuestLog(uid, suppressClick)
	local id = self.quests[uid] and self.quests[uid].id
	if QuestLogFrame:IsVisible() then
		if GetQuestLogSelection() == id then
	...
	if (numEntries > QUESTS_DISPLAYED) then
		if (id <= numEntries - QUESTS_DISPLAYED) then
			FauxScrollFrame_SetOffset(QuestLogListScrollFrame, id - 1)
			QuestLogListScrollFrameScrollBar:SetValue((id - 1) * QUESTLOG_QUEST_HEIGHT)
		else
	...
	-- Set the selection, at last:
	SelectQuestLogEntry(id)
	QuestLog_SetSelection(id)
	
	-- Call the onclick, just to help other addons out.
	if not suppressClick then
		_G["QuestLogTitle"..(id - FauxScrollFrame_GetOffset(QuestLogListScrollFrame))]:Click()
	end
end
